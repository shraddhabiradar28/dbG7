 ...
 * against two APIs that do not exist anywhere in the codebase:
 *
 *   - TradeEvent.created(String, ObjectNode)          — TradeEvent has no
 *                                                        static factories at all
 *   - ReconciliationEngine.scheduleRecon(String)      — no such method
 *
 * This broke test-compile, so the whole backend build failed. Commented out
 * rather than deleted so no work is lost.
 *
 * TO RE-ENABLE (@shraddhabiradar28): add the two methods above (they are what
 * tickets 142-144 were meant to introduce), then uncomment this file. It also
 * needs the DLQ consumer, currently disabled in
 * src/main/java/com/dbtraining/reconx/kafka/DlqConsumer.java.
 * ============================================================================
 */
